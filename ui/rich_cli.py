...
class RichCLI:
    ...
    def _update_status_for_event(self, status):
        def update(event: dict) -> None:
            ...
            elif event.get("type") == "tool_routing":
                status.update("[bold cyan]正在判断是否需要读取本地信息...[/bold cyan]")
            elif event.get("type") == "tool_planning":
                status.update("[bold cyan]正在规划需要读取的信息...[/bold cyan]")
            elif event.get("type") == "tool_running_command":
                status.update(f"[bold cyan]正在运行命令: {event.get('command', '')}[/bold cyan]")
            elif event.get("type") == "tool_changing_dir":
                status.update(f"[bold cyan]正在切换目录: {event.get('command', '')}[/bold cyan]")
